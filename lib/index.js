...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  for(const proxy of proxies) {
    const {authorizationCheck, httpsAgent, method, route, targetApi} = proxy;
    ...
    app[methodLowerCase](
      ...
      asyncHandler(async (req, res) => {
        ...
        // check authorization
        const {authorized} = await authorizationCheck({req});
        if(!authorized) {
        ...
        if(bodyMethods.has(methodLowerCase)) {
          ...
          request = targetApi[methodLowerCase]('', req.query, {httpsAgent});
        }
